...
def test_imports():
    ...
    try:
        from experiment import train_mujoco, play, plot
        print("✅ All experiment modules")
    except ImportError as e:
        print(f"❌ Experiment modules: {e}")
        return False
    
    try:
        from algorithms import ddpg, rollout_mujoco
        print("✅ All algorithm modules")
    except ImportError as e:
        print(f"❌ Algorithm modules: {e}")
        return False
    ...
    for demo_file in demo_files:
        if os.path.exists(demo_file):
            try:
                ...
                print(f"✅ {demo_file}: {data['obs'].shape[0]} trajectories")
            except Exception as e:
                print(f"❌ {demo_file}: {e}")
                return False
        else:
...
def test_training():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            from experiment.train_mujoco import launch
...
if __name__ == '__main__':
    ...
    if success:
        print("\n🎯 Basic tests passed! Attempting training test...")
